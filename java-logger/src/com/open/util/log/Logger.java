...
public final class Logger{
            ...
            if(null != mLogConfig){
                if(mLogConfig.isEnable) {
                    ...
                    if((mLogConfig.common_mode & LogConfig.LOG_MODE_FILE) == LogConfig.LOG_MODE_FILE){
                        mLogger.addLogger(new FileLogger(mLogConfig.file_path,mLogConfig.file_name_formater,mLogConfig.file_size,mLogConfig.file_syn));
                    }
    ...
    public static LogConfig updateConfig(String config_file_path){
        destroy();
    ...
    private static boolean fillTraceNames(String[] names){
        ...
        if (sts != null) {
            for (StackTraceElement st:sts) {
                ...
                names[ILog.INDEX_PID]        = ""+getPid();
                names[ILog.INDEX_TID]        = ""+Thread.currentThread().getId();
                return true;
            }
        }
    ...
    private static String transformMessage(Object input){
        try{
            if(input instanceof String){
                ...
                return getStackTraceString((Throwable) input);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
